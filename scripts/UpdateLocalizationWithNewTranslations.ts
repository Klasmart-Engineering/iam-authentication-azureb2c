...
const UpdateLocalizationWithNewTranslations = (language: any) => {
    ...
    fs.readFile(
        LOCALIZATION_XML_PATH, function (err, buffer) {
            ...
            xmlToJsonObject.TrustFrameworkPolicy.BuildingBlocks.Localization.LocalizedResources = xmlToJsonObject.TrustFrameworkPolicy.BuildingBlocks.Localization.LocalizedResources.filter(
            ...
            for (const child in localizationNewTranslation.custom_policy) {
                ...
                for (const secondChild in localizationNewTranslation.custom_policy[child]) {
                    if (secondChild !== "LocalizedCollection") {

                        for (const thirdChild in localizationNewTranslation.custom_policy[child][secondChild]) {
                            const localizedString = {} as LocalizedString;
                            localizedString.ElementType = secondChild;
                            const thirdChildObj = localizationNewTranslation.custom_policy[child][secondChild][thirdChild];
                            if (typeof thirdChildObj === 'object' && thirdChildObj !== null) {
                                const lastChild = Object.keys(thirdChildObj);
                                localizedString.ElementId = thirdChild
                                localizedString.StringId = lastChild[0]
                                localizedString.$t = localizationNewTranslation.custom_policy[child][secondChild][thirdChild][lastChild[0]]
                            }
                            else {
                                localizedString.StringId = thirdChild
                                localizedString.$t = localizationNewTranslation.custom_policy[child][secondChild][thirdChild]
                            }

                            if (!localizedResource.LocalizedStrings) {
                                localizedResource.LocalizedStrings = {} as LocalizedStrings;
                                localizedResource.LocalizedStrings.LocalizedString = new Array<LocalizedString>();
                            }
                            localizedResource.LocalizedStrings.LocalizedString.push(localizedString)
                        }
                        ...
                        for (const thirdChild in localizationNewTranslation.custom_policy[child][secondChild]) {
                            ...
                            const newTranslationObj = localizationNewTranslation.custom_policy[child][secondChild][thirdChild]
                            localizedCollection.Item.Value = Object.keys(newTranslationObj)[0];
                            localizedCollection.Item.Text = newTranslationObj[Object.keys(newTranslationObj)[0]];
                            if (!localizedResource.LocalizedCollections) {
                                localizedResource.LocalizedCollections = {} as LocalizedCollections;
                            }
                            localizedResource.LocalizedCollections.LocalizedCollection = localizedCollection
                        }
                    }
                }

                xmlToJsonObject.TrustFrameworkPolicy.BuildingBlocks.Localization.LocalizedResources.push(localizedResource);
            }
